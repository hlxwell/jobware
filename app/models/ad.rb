...
class Ad < ActiveRecord::Base
  ...
  state_machine :initial => :unapproved do
    ...
    after_transition :on => :approve do |ad|
      ...
      CompanyMailer.delay.ad_approval(ad.company, ad)
    end
    ...
    after_transition :on => :close do |ad|
      unless ad.available?
        ...
        CompanyMailer.delay.ad_expired(ad.company, ad)
      end
    end
  ...
  validates_length_of :name, :within => 5..30, :message => "长度必须控制在5-30个字以内。", :if => Proc.new { |ad| ad.display_type != AdPositionType::FAMOUS_COMPANY }
  validate :check_time
