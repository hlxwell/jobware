...
class Ad < ActiveRecord::Base
  ...
  state_machine :initial => :unapproved do
    ...
    after_transition :on => :approve do |ad|
      ...
      CompanyMailer.ad_approval(ad.company, ad).deliver if ad.company
    end
    ...
    after_transition :on => :close do |ad|
      unless ad.available?
        ...
        CompanyMailer.ad_expired(ad.company, ad).deliver if ad.company
      end
    end
  ...
  validates_length_of :name, :within => 3..30, :message => "长度必须控制在3-30个字以内。", :if => Proc.new { |ad| ad.display_type != AdPositionType::FAMOUS_COMPANY }
  validate :check_time
